...
const ProductDetails: React.FC = () => {
  ...
  const [userRating, setUserRating] = useState<number>(0); // <-- State for star rating
  const [reviewText, setReviewText] = useState<string>(''); // <-- State for review text
  const [submittedReviews, setSubmittedReviews] = useState<any[]>([]); // <-- State to hold submitted reviews (for demo)
  ...
  const fetchProduct = useCallback(async (combinedId: string) => {
    ...
    setUserRating(0); // Reset review form state on new product load
    setReviewText('');
    setSubmittedReviews([]); // Clear previous product's submitted reviews
  ...
  const handleReviewSubmit = (e: React.FormEvent) => {
    ...
    setSubmittedReviews(prev => [newReview, ...prev]); // Add to the top
     ...
     <div className="container mx-auto px-4 py-8">
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
         ...
         <div className="space-y-6">
           {/* ... (Keep Title, Description, Rating, Price, Brand/Category sections) ... */}
            <div>
           ...
           <Card>
             <CardContent className="p-4">
               <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                 ...
                 <div className="flex items-center gap-2"><Shield className="h-4 w-4 text-blue-600 flex-shrink-0" /><span>{product.specifications.Warranty || '1 year warranty'}</span></div>
       ...
       <div className="mt-12">
         <Tabs value={activeTab} onValueChange={setActiveTab}>
           <TabsList className="grid w-full grid-cols-3"> {/* Corrected class */}
            ...
            <Card>
              <CardContent className="p-6">
                <div className="space-y-8"> {/* Increased spacing */}
                  ...
                  {/* Existing Reviews Section (Mocked) */}
                  <div>
                    <h3 className="text-xl font-bold mb-4">Customer Reviews</h3>
                    <div className="flex items-center gap-8 mb-6">
                       <div className="text-center">
                         <div className="text-4xl font-bold">{product.rating}</div>
                         <div className="flex items-center gap-1 justify-center"> {/* Centered stars */}
                       ...
                       {/* Optional: Add rating breakdown bars here */}
                     </div>
                      ...
                      <div key={review.id} className="border rounded-lg p-4 mb-4 bg-muted/50">
                        <div className="flex items-center gap-4 mb-2">
                           <div className="w-10 h-10 bg-secondary rounded-full flex items-center justify-center text-secondary-foreground font-semibold">
                             {review.author.substring(0, 1)} {/* Initial */}
                           </div>
                           <div>
                         ...
                         <p className="text-sm">{review.text}</p> {/* Use default foreground color */}
                      </div>
                    ))}
                        ...
                        <div className="flex items-center gap-1">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <Button
                              ...
                              className="p-1 text-gray-300 hover:text-yellow-400 focus-visible:text-yellow-400" // Base color gray
                            >
                              <Star
                                className={`h-6 w-6 transition-colors ${
                                  star <= userRating
                                    ? 'fill-yellow-400 text-yellow-400' // Filled style when selected
                                    : ''
                                }`}
                              />
                            </Button>
